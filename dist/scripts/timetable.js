...
(function () {
    ...
    Timetable.prototype = {
        ...
        removeEvent: function (event, location) {
            if (!locationExistsIn(location, this.locations)) {
                throw new Error('Unknown location');
            } else {
                if (eventExistsIn(event, this.events)) {
                    var index = getIndexOfEvent(event, this.events, location);
                    if (index != -1) {
                        this.events.splice(index, 1);
                    } else {
                        throw new Error('Event dont belong to this location')
                    }
                } else {
